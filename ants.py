...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            # BEGIN Problem 13
            "*** YOUR CODE HERE ***"
            if type(insect) == QueenAnt and insect.ant_type:
                return
            # END Problem 13
...
class Bee(Insect):
    ...
    def action(self, colony):
        ...
        if not self.move_forward:
            ...
            if destination is colony.beehive:
                destination = None
        self.move_forward = True
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive, min_range = min_range, max_range = max_range):
        ...
        index = 0
        while copy != beehive:
            if len(copy.bees) != 0 and index >= min_range and index <= max_range:
    ...
    def action(self, colony):
        ...
        self.throw_at(self.nearest_bee(colony.beehive, self.min_range, self.max_range))
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor<= 0:
            dam = self.damage + amount
        copy = self.place.bees[:]
        for bee in copy:
            bee.reduce_armor(dam)
        if self.armor <= 0:
            self.place.remove_insect(self)
            self.death_callback()
        # END Problem 5
...
# The WallAnt class
class WallAnt(Ant):
    """WallAnt does nothing each turn but with larger armor value"""
...
class ScubaThrower(ThrowerAnt):
    """A type of ThrowerAnt that is watersafe"""
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        ...
        "*** YOUR CODE HERE ***"
        Ant.__init__(self, armor)
        self.ant_type = not QueenAnt.has_init
        QueenAnt.has_init = True
        ...
        if not self.ant_type:
            ...
            while ex:
                if ex.ant is None:
                    ex = ex.exit
                    continue
                elif ex.ant.is_container:
                    if not ex.ant.has_double:
                        ex.ant.damage *= 2
                        ex.ant.has_double = True
                    if ex.ant.contained_ant and not ex.ant.contained_ant.has_double:
                        ex.ant.contained_ant.damage *= 2
                        ex.ant.contained_ant.has_double = True
                elif not ex.ant.has_double:
                    ex.ant.damage *= 2
                    ex.ant.has_double = True
                ex = ex.exit
        # END Problem 13
    ...
    def reduce_armor(self, amount):
        ...
        if self.ant_type and self.armor <= 0:
            bees_win()
        # END Problem 13
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem EC
        "*** YOUR CODE HERE ***"
        if target and not target.has_scared:
            target.has_scared = True
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        ...
        distance = 0
        while pl:
            if pl.ant and pl.ant != self:
                result[pl.ant] = distance
            if pl.bees:
                for bee in pl.bees:
                    result[bee] = distance
            distance += 1
